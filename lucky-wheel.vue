<template>
    <div class="container">
        ...
        <div class="main">
            ...
            <div class="bg-p"></div>
            <div class="content">
                <div class="lottery_ticket">今日免费抽奖次数： {{ lottery_ticket >= 0 ? lottery_ticket : 0}}</div>
            </div>
            <div class="tip">
        ...
        <div class="toast" v-show="toast_control">
            <div class="toast-container">
                ...
                <div class="toast-title" v-if="lottery_ticket >= 0">
                    {{toast_title}}
                </div>
                <div class="toast-title" v-else>
                    今日抽奖机会已用完
                </div>
                <div class="toast-btn">
...
export default {
  ...
  computed: {
    toast_title() {
      return this.hasPrize
              ? "恭喜您，获得" +this.prize_list[this.priceIdx].count + ' ' + this.prize_list[this.priceIdx].name : "未中奖";
    },
    toast_pictrue() {
      return this.hasPrize
  ...
  methods: {
    ...
    rotate_handle() {
      ...
      //每次请点击发送请求前先让转盘转动起来
      this.rotate_angle = "rotate("+ (this.start_rotating_degree + 180) +"deg)";
      t = setTimeout(() => {
        this.rotating();
        clearTimeout(t);
      }, 2000)
    ...
    rotating(index) {
      ...
      if (type == 0) {
        ...
        // 转动盘子 = 上一次转动了总的度数(衔接上一次旋转的角度) + 每一次固定转的度数 + 当次转动度数 - 上一次相对初始位置转了多少度（0 <= initDeg <= 337.5） 
        var rotate_angle =
    ...
    game_over(idx) {
      this.toast_control = true;
      this.priceIdx = idx;
      this.hasPrize = this.prize_list[idx].isPrize
    },
    //关闭弹窗
    close_toast() {
...
.rotate-div {
  /*animation: rotating 2s ease infinite forwards;*/
